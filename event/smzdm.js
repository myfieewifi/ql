...
let userCookie = ($.isNode() ? process.env.smzdmCookie : $.getdata('smzdmCookie')) || '';
let userCookieArr = []
...
async function SignSmzdm() {
    ...
    if(result.success == true) {
        console.log(`什么值得买签到成功! 签到天数: ${result.data.data.checkin_num} | Lv:${result.data.data.rank} | 经验值:${result.data.data.exp}`)
    } else {
        console.log(`什么值得买签到失败：${result.data.error_msg}`)
    }
}
...
function populatePostUrl(url,reqBody){
    ...
    let urlObject = {
        ...
        headers: {
            ...
            'User-Agent' : 'smzdm 10.2.10 rv:112 (iPhone 12; iOS 15.0; zh_CN)/iphone_smzdmapp/10.2.10',
            'Cookie' : userCookieArr[userIdx],
        },
        body: reqBody
    }
    return urlObject;
